...
last_synced_commit: 8f9f5c20a877cb0ffcf8c5d1781f171a0b6ff1a9
---
...
**Versioning:** v2.2.2. `.version-bump.json` syncs version across `package.json` + `.claude-plugin/plugin.json` + `marketplace.json` (`plugins.0.version`). `.cursor-plugin/plugin.json` + `CLAUDE.md` Project Version tracked manually. Stamp `plugin_version` on every artifact mutation.
...
**Generated artifacts portable + standalone.** Created pipeline = drop into any project after creation. `parity-test-*` dirs (`.claude/` a,b · `.opencode/` i,j · `.codex/` e,f · `.agents/antigravity/` c,d) = EXAMPLE generated pipelines from dry-run tests. **Codex native-layout migration (#50, v2.2.2):** codex artifacts moved `.agents/codex/` -> `.codex/` + skills under `.agents/skills/`; `.codex-plugin/` manifest retained (discovery-only). NOT part of shipped plugin bundle. Demo of portability. Future direction: curate generic useful example pipelines so users learn + lift into own projects.
...
**Failure-mode honesty:** structural guardrails (isolation, iter cap, profile inversion) = mostly PREVENTIVE — EXCEPT two real reported flaws now fixed: worktree artifact-loss (#31, v2.1.1) + audit-report-ownership / `permissionMode` split-brain (#33, v2.1.2). Plus run-safety audit gate (#37): `running-a-pipeline` Phase 0.7 pre-run tripwire HARD-STOPs a version-drifted pipeline carrying worktree artifact-loss (#23/#24) before dispatch. Plus phase-skip hardening (#45, v2.2.1): live orchestration could drift phase order, invent a fake "no-active-run" phase, skip 0.6/0.7 pre-dispatch -> no runtime observer caught it (auditor checks defs, CI checks JSON, guardrails self-enforced by drifting model). Fix in `running-a-pipeline`: Phase Ordering Contract lifted to top of protocol (total order stated verbatim, no fake phase exists) + mandatory phase-manifest `TodoWrite` at Phase 0 (skip = user-visible) + Phase 3 dispatch precondition HARD-STOPs if 0.6/0.7 absent from phase ledger (persisted to `metadata.phases_executed` at Phase 2 -> resume-safe). Plus invocation-flag trap fix (#49): a skill loaded via the Skill tool at runtime MUST NOT carry `disable-model-invocation: true` (it blocks the load -> guaranteed-fail "cannot be used with Skill tool"). 6 skills carried it wrongly. `running-a-pipeline` Phase 0.25 attempted `Skill(sk-platform-dispatch).DETECT()` -> fatal during `/run-pipeline`; instructional try/catch recovery not honored by model -> fixed Read-first (Read dispatch body + run DETECT() inline; Task-probe + INLINE-DETECT fallback preserved; flag kept since Read bypasses it). 5 siblings loaded via Skill tool (sk-pipeline-grilling — ACTIVE bug, `creating-a-pipeline` grilling gate "load via Skill tool" failed every `/new-pipeline`; sk-pipeline-paths method-calls; routing-table sk-spec-driven-development / sk-claude-code-conventions / sk-4d-method) -> drop flag, keep `user-invocable: false`, aligning to `sk-model-resolver` precedent (Skill-loaded, no flag). Agent `skills:`-list loading is flag-immune, so skills loaded ONLY via agent lists (sk-write-review-isolation, sk-pipeline-patterns, sk-dynamic-routing, sk-hashline-protocol, sk-rationalization-resistance) correctly KEEP the flag. `PARITY_TESTING: MANUAL_PHASE1` — no automated cross-platform gate. Cross-platform parity = manual per-platform.
