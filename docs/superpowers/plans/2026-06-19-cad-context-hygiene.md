...
- Optional create: `docs/superpowers/evidence/2026-06-19-cad-context-hygiene.md`  
  Records before/after description lengths, stale-phrase hit counts, compatibility matrix, and false-positive/expected-invocation checks.
...
### Task 6: Package Sync and Final Verification
...
- Read: changed files from `git diff HEAD~6..HEAD --name-only`
...
Expected: changed files cover router/metadata, creation/architect CAD contract, auditor rules, fixtures, packaged copies, and evidence.
...
- [ ] **Step 3: Verify packaged source parity one more time**
...
- [ ] **Step 4: Commit any self-review fixes**

If Task 7 reveals fixes, make them and commit:
...
- Spec coverage: Each spec area maps to at least one task: compatibility/evidence in Task 1, router/invocation in Task 2, CAD generation in Task 3, audit rules in Task 4, fixtures/tests in Task 5, packaging and measurements in Task 6, final verification in Task 7.
