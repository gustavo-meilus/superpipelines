...
- **Description:** Write the `extractor.toml` TOML agent file and its companion `extractor-protocol/SKILL.md`. The TOML file must have no body text — only TOML key-value pairs. Protocol skill supplies full operational instructions.
...
- **Acceptance:** `extractor.toml` is valid TOML with `model = "gpt-5.4-mini"`, `model_reasoning_effort = "medium"`, `sandbox_mode = "workspace-write"`, `version = "1.0.0"`, `plugin_version = "2.0.0"`. `extractor-protocol/SKILL.md` has `disable-model-invocation: true` and `user-invocable: false`. No `---` frontmatter block in the TOML file.
- **Depends on:** T-1
- **Estimated turns:** 10 min

## T-3: Write formatter agent TOML + protocol skill

- **Description:** Write the `formatter.toml` TOML agent file and its companion `formatter-protocol/SKILL.md`. Same TOML format rules as T-2.
...
- **Acceptance:** `formatter.toml` is valid TOML with `model = "gpt-5.4-mini"`, `model_reasoning_effort = "medium"`, `sandbox_mode = "workspace-write"`, `version = "1.0.0"`, `plugin_version = "2.0.0"`. `formatter-protocol/SKILL.md` has `disable-model-invocation: true` and `user-invocable: false`.
- **Depends on:** T-1
- **Estimated turns:** 10 min
