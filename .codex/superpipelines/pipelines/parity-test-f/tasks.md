...
- **Acceptance:** All directories exist; `ls {ROOT}/agents/superpipelines/parity-test-f/` returns `analyzer.toml reviewer.toml reporter.toml`.
- **Depends on:** none
- **Estimated turns:** 5 min

## T-2: Write analyzer agent TOML + protocol skill

- **Description:** Write the `analyzer.toml` TOML agent file and its companion `analyzer-protocol/SKILL.md`. The TOML file must include scalar inline `instructions = """..."""` prompt content plus Codex metadata key-value pairs.
...
- **Acceptance:** `analyzer.toml` is valid TOML with `name = "analyzer"`, `description`, `model = "gpt-5.4-mini"`, `model_reasoning_effort = "medium"`, `sandbox_mode = "workspace-write"`, and scalar `instructions = """..."""`. It must not contain `[[developer_instructions]]`. `analyzer-protocol/SKILL.md` has `disable-model-invocation: true` and `user-invocable: false`. No `---` frontmatter block in the TOML file.
- **Depends on:** T-1
- **Estimated turns:** 10 min

## T-3: Write reviewer agent TOML + protocol skill

- **Description:** Write the `reviewer.toml` TOML agent file and its companion `reviewer-protocol/SKILL.md`. CRITICAL: `sandbox_mode = "read-only"` is MANDATORY for the reviewer per the Tier 1d `reviewer_isolation_recipe`. The TOML file must include scalar inline `instructions = """..."""` prompt content plus Codex metadata key-value pairs.
...
- **Acceptance:** `reviewer.toml` is valid TOML with `name = "reviewer"`, `description`, `model = "gpt-5.5"`, `model_reasoning_effort = "high"`, `sandbox_mode = "read-only"` (NOT "workspace-write"), and scalar `instructions = """..."""`. It must not contain `[[developer_instructions]]`. `reviewer-protocol/SKILL.md` has `disable-model-invocation: true` and `user-invocable: false`.
- **Depends on:** T-1
- **Estimated turns:** 10 min

## T-4: Write reporter agent TOML + protocol skill

- **Description:** Write the `reporter.toml` TOML agent file and its companion `reporter-protocol/SKILL.md`. Same scalar inline `instructions = """..."""` TOML format rules as T-2.
...
- **Acceptance:** `reporter.toml` is valid TOML with `name = "reporter"`, `description`, `model = "gpt-5.4-mini"`, `model_reasoning_effort = "medium"`, `sandbox_mode = "workspace-write"`, and scalar `instructions = """..."""`. It must not contain `[[developer_instructions]]`. `reporter-protocol/SKILL.md` has `disable-model-invocation: true` and `user-invocable: false`.
- **Depends on:** T-1
- **Estimated turns:** 10 min
