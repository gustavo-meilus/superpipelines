...
- **Discovery: single-root by default, gated legacy fallback (Q5, #64 collapse).** Call `sk-pipeline-paths.ENUMERATE_PIPELINE_ROOTS(workspace)`. By default this is **two reads** — `<workspace>/.superpipelines/registry.json` + `~/.superpipelines/registry.json` (both `layout: "data"`) — with **no per-tier loop** (design spec §4). The 5 per-tier legacy scope roots (`layout: "legacy"`) are appended ONLY when `LEGACY_BACK_COMPAT_ARMED` detects a pre-v2 old-root dir; an all-data workspace never enumerates them. This is the "Phase 0 stops re-deriving location every run" win — back-compat reads are preserved (design spec §8) but no longer the default mechanic.
...
- **Resume scan: single-root by default, gated legacy tail (Q5, #64 collapse)**: For a `layout: "data"` pipeline, the resume scan is the single `DATA_ROOT/temp/{P}/` (workspace + user) — no per-tier loop. The legacy scope roots are scanned at `<root>/superpipelines/temp/{P}/` ONLY for those roots Phase 0's `ENUMERATE_PIPELINE_ROOTS` returned (i.e. when `LEGACY_BACK_COMPAT_ARMED` fired); this preserves resume of old-root state files written under a different tier's scope root (e.g., a CC-scaffolded legacy pipeline resumed from Cursor) without re-deriving all 5 tiers on an all-data workspace.
