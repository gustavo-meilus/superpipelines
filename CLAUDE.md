...
- `PERMISSION_MODE: PER_AGENT` — Agents declare explicit permission boundaries (e.g., `acceptEdits`, `plan`) in frontmatter. **Plugin-scope caveat (Claude Code):** CC ignores `permissionMode` (and `hooks`, `mcpServers`) on plugin-shipped subagents, so for the bundle's own `agents/*.md` the structural barrier is `tools:` + `disallowedTools:` alone; `permissionMode` there is declarative intent. Materialized per-pipeline agents under `.claude/agents/superpipelines/{P}/` are project-scope, where `permissionMode` IS enforced — `TRANSLATE_CAD_TO_CC` keeps emitting it. Documentation claiming `permissionMode` enforcement on the plugin path is a defect (auditor BUNDLE-08).
- `STATE_MANAGEMENT: STRUCTURED_JSON` — State persists to the single data root: `<workspace-or-user>/.superpipelines/temp/{P}/{runId}/pipeline-state.json` (resolved by `RESOLVE_DATA_ROOT(scope)`). State carries `plugin_version`, `metadata.source_tier`, and `metadata.runtime_tier` from the run start. Legacy per-tier roots (`.claude/superpipelines/…`, etc.) remain a read-only back-compat resume path for one major; see `ARTIFACT_PORTABILITY` and ADR-0003.
...
- `ARTIFACT_PORTABILITY: DATA_ONLY_PORTABLE_ALL_TIERS` — Generated pipelines are tool-neutral data under a single `.superpipelines/` root (Canonical Agent Defs + step protocols + entry body), so a pipeline scaffolded on any tier is copy-paste portable across tools and projects. At dispatch the orchestrator materializes each CAD into the host's native agent dialect (CC/OpenCode/Codex structurally; Antigravity/Tier 2 convention-only with surfaced degradation). This upgrades the former `OC_NOT_PORTABLE` constraint — OpenCode pipelines now materialize structurally from the same CAD as every other Tier 1 host. See ADR-0003.
- `PARITY_TESTING: TRANSLATION_AUTOMATED_DISPATCH_MANUAL` — The CAD → native translation layer is gated in CI: `scripts/check-materialization-parity.js` reproduces the golden fixtures (`cc-materialize`, `oc-materialize`, `codex-materialize`) on every PR; a translator/golden divergence fails the build, and the fixture decides (translator pseudocode in `sk-platform-dispatch` and the goldens change together). Live-host dispatch verification remains a manual release-checklist step until the per-host probes land (fix-plan WI-11/WI-12/WI-14).
...
- **Generated Artifacts**: Pipelines are written as tool-neutral **data** under the single `.superpipelines/` root — `<workspace>/.superpipelines/` (`project`/`local` scope) or `~/.superpipelines/` (`user` scope), resolved by `sk-pipeline-paths` (`RESOLVE_DATA_ROOT`). Per-tier roots are read-only back-compat only; ephemeral materialized native agent files are disposable cache (see `ARTIFACT_PORTABILITY`, ADR-0003).
...
<glossary>
  ...
  <term name="Data Root">The single tier-independent base directory for all generated pipeline data: `<workspace>/.superpipelines/` or `~/.superpipelines/` (`RESOLVE_DATA_ROOT`). Since v2.3.0 this is where every generated artifact lives.</term>
  <term name="Scope Root">A per-tier base directory (`.claude/`, `.opencode/`, …) resolved by `RESOLVE_SCOPE_ROOT`. No longer the artifact location — consumed only for read-only legacy back-compat and as the native materialization target at dispatch.</term>
</glossary>
...
Issues tracked in GitHub Issues (`gustavo-meilus/superpipelines`) via the `gh` CLI. See `docs/agents/issue-tracker.md`.

### Triage labels

Canonical triage roles map 1:1 to identically-named GitHub labels. See `docs/agents/triage-labels.md`.
